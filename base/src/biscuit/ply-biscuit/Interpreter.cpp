...
FnResult eval(Interpreter::StackFrame* frame, const Expression* expr) {
    ...
    switch (expr->id) {
        ...
        case Expression::ID::NameLookup: {
            Label name = expr->name_lookup()->name;

            // Check local variables first.
            if (AnyObject* value = frame->local_variable_table.find(name)) {
                base->return_value = *value;
                return Fn_OK;
            }

            // Then client.
            base->return_value = frame->interp->resolve_name(name);
            if (base->return_value)
                return Fn_OK;
        ...
        case Expression::ID::InterpolatedString: {
            const Expression::InterpolatedString* string_exp =
                expr->interpolated_string().get();

            // Concatenate all the pieces.
            MemOutStream mout;
            for (const Expression::InterpolatedString::Piece& piece :
                 string_exp->pieces) {
                mout << piece.literal;
                if (piece.embed) {
            ...
            // Return the result.
            AnyObject* string_obj = base->local_variable_storage.append_object(
        ...
        case Expression::ID::BinaryOp: {
            const Expression::BinaryOp* binary_op = expr->binary_op().get();
        ...
        case Expression::ID::UnaryOp: {
            const Expression::UnaryOp* unary_op = expr->unary_op().get();
        ...
        case Expression::ID::Call: {
            const Expression::Call* call = expr->call().get();
            ...
            for (const Expression* arg_expr : call->args) {
                ...
                if (!is_return_value_on_top_of_stack(base)) {
                    // The return value is not a temporary object, and the interpreter
                    // is currently designed to pass all arguments "by value", like in
                    // C, so we should make a copy here. (In the future, we could extend
                    // the interpreter to support passing "by reference" as well, like
                    // in C++. In the meantime, scripts can achieve the same thing by
                    // passing pointers.)
                    arg = base->local_variable_storage.append_object(
            ...
            if (const auto* function_def =
                    callee.safe_cast<Statement::FunctionDefinition>()) {
                // Function is implemented in script.
                PLY_ASSERT(!self); // Not possible to define classes in script yet
                PLY_ASSERT(args.num_items() ==
                           function_def->parameter_names.num_items());
            ...
            base->error(String::format("cannot call '{}' as a function",
                                       callee.type->get_name()));
            return Fn_Error;
        }
    ...
    for (const Statement* statement : block->statements) {
        ...
        switch (statement->id) {
            ...
            case Statement::ID::If_: {
                const Statement::If_* if_ = statement->if_().get();
                // Evaluate condition.
                ...
                const StatementBlock* block =
                    (was_true ? if_->true_block : if_->false_block);
                if (block) {
                    result = exec_block(frame, block);
                    if (result != Fn_OK)
                        return result;
                }
                break;
            ...
            case Statement::ID::While_: {
                const Statement::While_* while_ = statement->while_().get();
                for (;;) {
                    // Evaluate condition.
                    ObjectStack::Boundary local_variable_storage_boundary =
                        base->local_variable_storage.end();
            ...
            case Statement::ID::Assignment: {
                const Statement::Assignment* assign = statement->assignment().get();
                ObjectStack::Boundary local_variable_storage_boundary =
                    base->local_variable_storage.end();
                ...
                if (assign->left->id == Expression::ID::NameLookup) {
                    ...
                    AnyObject* value =
                        frame->local_variable_table.insert_or_find(name, &was_found);
                    if (!was_found) {
                        if (is_return_value_on_top_of_stack(base)) {
                            ...
                            *value = base->local_variable_storage.items.tail();
                            ...
                            AnyObject* dest =
                                base->local_variable_storage.append_object(
                                    base->return_value.type);
