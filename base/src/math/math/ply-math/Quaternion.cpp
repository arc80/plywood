...
PLY_NO_INLINE Quaternion Quaternion::from_unit_vectors(const Float3& start,
    ...
    if (w < 1e-6f) {
        ...
        Float3 not_collinear =
            (fabsf(start.x) < 0.9f) ? Float3{1, 0, 0} : Float3{0, 1, 0};
        Float3 axis = cross(start, not_collinear);
        return Float4{axis, 0}.normalized().as_quaternion();
    }
    Float3 v = cross(start, end);
    return Float4{v, w}.normalized().as_quaternion();
}
